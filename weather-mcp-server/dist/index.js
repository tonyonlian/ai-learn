 ...
 */
async function geocodeCity(place, sheng) {
    const params = {
        name: place,
        count: "5",
        language: "zh",
 ...
 */
function formatMarkdown(data, location, forecastDays) {
    const lines = [];
    ...
    for (let i = 0; i < Math.min(daily.time.length, forecastDays); i++) {
        ...
        lines.push(`| ${date} | ${w.icon} ${w.cn} | ${daily.temperature_2m_max[i]}°C | ${daily.temperature_2m_min[i]}°C | ${daily.precipitation_sum[i]}mm | ${daily.uv_index_max[i]}(${getUVLevel(daily.uv_index_max[i])}) | ${(daily.wind_speed_10m_max[i] / 3.6).toFixed(1)}m/s |`);
    }
    lines.push("");
 ...
 */
function formatJson(data, location, forecastDays) {
    const now = data.current;
    const daily = data.daily;
    return {
        location: {
...
server.registerTool("weather_get_forecast", {
    title: "查询城市天气预报",
...
Error Handling:
  ...
  - 返回 "错误：请求超时" 如果网络问题`,
    ...
    try {
        ...
        const data = await fetchWeather(geo.latitude, geo.longitude, params.forecast_days);
